...
export function ProfileManager() {
  ...
  return (
    <div className="rounded-lg bg-white p-4 shadow-sm space-y-3">
          ...
          <div key={p.id} className="flex items-center justify-between rounded-xl border border-slate-200 px-3 py-2">
            <div>
              <div className="text-sm font-medium">
                {p.name}{' '}
                {p.id === currentProfileId ? <span className="text-xs font-normal text-slate-600">(current)</span> : null}
              </div>
            ...
            <div className="flex gap-2">
              {p.id !== currentProfileId ? (
                <button
                  className="rounded-xl border border-slate-300 bg-white px-3 py-1 text-xs hover:bg-slate-50 disabled:opacity-50"
                  onClick={() =>
                    void (async () => {
                      setBusy(true)
                      try {
      ...
      <Link to="/onboarding" className="inline-block rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm hover:bg-slate-50">
        Create new profile
      </Link>
    </div>
  )
