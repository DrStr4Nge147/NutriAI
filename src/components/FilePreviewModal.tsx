...
export function FilePreviewModal(props: {
  ...
  const content = (
    ...
    >
      ...
      <div className="relative flex w-full max-w-4xl max-h-[calc(100dvh-2rem)] flex-col overflow-hidden rounded-2xl bg-white p-4 shadow-2xl dark:bg-slate-900">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="truncate text-sm font-semibold text-slate-900 dark:text-slate-100">{props.file.name}</div>
            <div className="mt-1 text-xs text-slate-600 dark:text-slate-300">{props.file.mimeType}</div>
          </div>
          <button
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            {canZoom ? (
              <>
                <button
                  className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-slate-300 bg-white text-slate-800 hover:bg-slate-50 disabled:opacity-50 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:hover:bg-slate-900"
                  onClick={zoomOut}
                  type="button"
                  disabled={zoom <= 0.5}
                  aria-label="Zoom out"
                ...
                <div className="text-xs text-slate-600 dark:text-slate-300">{Math.round(zoom * 100)}%</div>
              </>
            ) : null}
          </div>
        </div>
        ...
        <div
          ...
          className="mt-3 min-h-0 flex-1 overflow-hidden rounded-xl border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-950"
        >
          {kind === 'image' ? (
            <div
            ...
            <div className="p-2 text-sm text-slate-700 dark:text-slate-200">Preview not supported for this file type.</div>
          )}
        </div>
      </div>
    </div>
